...
def player_v2():
    if not os.path.exists(PLAYER_V2_PATH):
        return "player_v2.html not found inside data folder", 404
...
def probe():
    ...
    return {
        "type": "hls" if ".m3u8" in url.lower() else "ts",
        "ip_blocked": False
    }
...
def rewrite_m3u8(text, base_url):
    ...
    for line in text.splitlines():
        ...
        if s.startswith("#"):
            ...
            lines.append(
                re.sub(r'URI="([^"]+)"', repl, line)
            )

        else:
            lines.append(
                make_proxy_url(urljoin(base_url, s))
            )
