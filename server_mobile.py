...
def home() -> Response:
    ...
    if not os.path.exists(path):
        return Response(
            "player.html / player_v2.html not found inside data folder", status=404
        )
...
def player_v2() -> Response:
    if not os.path.exists(PLAYER_V2_PATH):
        return Response("player_v2.html not found", status=404)
...
def probe() -> Response:
    ...
    if not is_http_url(url):
        return make_response(
            {"type": "hls", "ip_blocked": False, "error": "invalid url"}, 400
        )
    ...
    try:
        ...
        return make_response(
            {"type": "hls" if ".m3u8" in u else "ts", "ip_blocked": False},
            200,
        )
